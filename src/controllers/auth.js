...
const { getUserDevice, editUser } = require('../models/user');
	...
	postLogin: async function(req, res) {
		...
		try {
			...
			if (check) {
				...
				let checkDevice = await getUserDevice(req.body.device);
				if (checkDevice.length) {
					await editUser(checkDevice[0].id, { device_token: null });
				}
				await editUser(result[0].id, { device_token: req.body.device });
